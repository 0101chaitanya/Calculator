...
function appendNum(inputX) {
    // console.log('hi', inputX);
    if (runningOperator == '') {
        if (stack == 0) input.textContent = '0';
        stack = parseFloat((stack) + inputX);

        input.textContent = input.textContent + inputX;
        console.table(stack, input.textContent);
  ...
  console.table(stack, input.textContent);  } else if (runningOperator == '-') {
        if (stack == 0) input.textContent = '0';
 ...
 console.table(stack, input.textContent);   } else if (runningOperator == '.') {
        ...
        stack = parseFloat(stack) + 0.1 * inputX;
        console.log(typeof stack);
        runningOperator = '';
          input.textContent = `
        $ { input.textContent }
 ...
 console.table(stack, input.textContent);   } else if (runningOperator == '=') {
        ...
        runningOperator = '';
console.table(stack, input.textContent);    }
     else if (runningOperator == 'backspace') {
        input.textContent ='0';
        stack = 0;
        console.log(typeof stack);
        runningOperator = '';
...
/*        function operatorPlace(arg) {
    ...
    /* filter(item => Object.values(item) == 'valid')
        .reduce((object) => {
            evalString = 0;
            //console.log(`
        $ { 0 } + $ { Number.parseFloat(Object.keys(object)).toFixed(4) }
        `);
            if (!isNaN(Number.parseFloat(Object.keys(object)).toFixed(4))) {
                evalString = `
        $ { evalString }
        $ { Number.parseFloat(Object.keys(object)).toFixed(4) }
        `;
                console.log(evalString);
            }
