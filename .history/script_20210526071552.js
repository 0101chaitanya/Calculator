...
computedArray = compute(input).filter(item => Object.values(item) == 'valid')
    .reduce(object => {
            console.log(Number.parseFloat(Object.keys(object)).toFixed(4))); console.log((!isNaN(Number.parseFloat(Object.keys(object)).toFixed(4))));
        //  if (!isNaN(object.parseFloat().toFixed(4)))
    });
/* .filter(item => {
    if(item.keys)
}); */
