...
obj.divFun = obj.str.slice().replaceAll('/', `div()`);
console.log(obj.divFun);
obj.mulFun = obj.divFun.slice().replaceAll('*', `mul()`);
console.log(obj.mulFun);
obj.addFun = obj.mulFun.slice().replaceAll('+', `add()`);
console.log(obj.addFun);
obj.subFun = obj.addFun.slice().replaceAll('-', `sub()`);
console.log(obj.subFun);
obj.arr = obj.subFun.split(/\D/g);
let regX;
/* for (i = 0; i < obj, subFun.length - 1; i++) {
