...
for (let i = obj.divStack.length - 1; i >= 0; i--) {
    obj.evaluatedArray[i] = obj.numArray.splice(parseFloat(obj.parsedDiv[i]), 2, obj.divStack[i]);
    obj.operatorArray.splice(parseFloat(obj.parsedDiv[i]), 1);
}
obj.mulStack = [];
let mulIndex = 0;
let mulAdjuster = 0;
    ...
    if (!isNaN(item)) {
        obj.parsedMul.forEach(function(value) {
                ...
                if (item < value) {
                    ...
                    value = value - 1;
                    console.log(value;)
                    obj.parsedMul.splice(obj.parsedMul.indexOf(item), 1, value = value - 1);
                )
...
console.log(obj.parsedMul, obj.parsedDiv, (obj.parsedDiv[0]));

obj.parsedMul.forEach(function(item) {
    obj.mulStack[divIndex] = obj.numArray[(item)] / obj.numArray[(item + 1)];

    mulIndex++;
});
//console.log(obj.evaluatedArray, obj);

for (let i = obj.mulStack.length - 1; i >= 0; i--) {
    obj.evaluatedArray[i] = obj.numArray.splice(parseFloat(obj.parsedMul[i]), 2, obj.divStack[i]);
    obj.operatorArray.splice(parseFloat(obj.parsedMul[i]), 1);
}
