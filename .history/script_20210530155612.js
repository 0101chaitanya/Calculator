...
let a = '-4*3.567+9/3-2';
let inputArray = a.split('');
let divideArray = a.split('/');
console.log(inputArray, divideArray);
let divideArray[0] = divideArray[0].split('').reverse();
console.log(firstArgument);
let num = 0;
leftFinder: for (let i = divideArray[0].length - 1; i >= 0; i--) {
    if (!isNaN(parseFloat(divideArray[0][i])) || divideArray[0][i] == '.') {
        num = (divideArray[0][i]) + num;
    } else { break leftFinder; }
    console.log(num);
}